...
function dynamicRequestFactory(httpMethod, path) {
  return async function makeRequest(...args) {
    const fullPath = `${this.path}${path}`;

    const httpM = httpMethod.toLowerCase();

    if (httpM === 'post') {
      return this._create(fullPath, ...args);
    } else if (httpM === 'get') {
      return this._get(fullPath, ...args);
    } else if (httpM === 'put') {
      return this._update(fullPath, ...args);
    } else if (httpM === 'delete') {
      return this._delete(fullPath, ...args);
    }
