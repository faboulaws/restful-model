...
class Model {
  ...
  _get(path, params, context) {
    const { query, endpoint } = this.service.getEndpointAndQuery(path, params);
    return this.service.get(`${endpoint}`, query, context);
  }
  ...
  async query(params, include = [], context = {}) {
    ...
    const responseData = await this._get(this.path, params, context);
    if (include.length) {
  ...
  async get(params, include = [], context = {}) {
    const responseData = await this._get(`${this.path}/:id`, params, context);
  ...
  _create(path, payload, query, context) {
    const { endpoint } = this.service.getEndpointAndQuery(path, payload);
   ...
   * @returns {Promise.<TResult>}
   */
  create(payload, query = {}, context = {}) {
    return this._create(this.path, payload, query, context);
  }

  _update(path, params, payload = {}, context = {}) {
    const { query, endpoint } = this.service.getEndpointAndQuery(path, params);
    return this.service.put(endpoint, payload, query, context);
  }

  update(params, payload, context = {}) {
    return this._update(`${this.path}/:id`, params, payload, context);
  }

  _delete(path, params, payload = {}, context = {}) {
    const { query, endpoint } = this.service.getEndpointAndQuery(path, params);
    return this.service.delete(endpoint, payload, query, context);
  }

  delete(params, payload = {}, context = {}) {
    return this._delete(`${this.path}/:id`, params, payload, context);
  }
